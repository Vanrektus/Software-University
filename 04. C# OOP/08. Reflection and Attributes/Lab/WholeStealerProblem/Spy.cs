...
namespace Stealer
{
    public class Spy
    {
        ...
        public string CollectGettersAndSetters(string classToInvestigate)
        {
            ...
            foreach (MethodInfo currentMethod in classMethods.Where(m => m.Name.StartsWith("get")))
            {
                sb.AppendLine($"{currentMethod.Name} will return {currentMethod.ReturnType}");
            }

            foreach (MethodInfo currentMethod in classMethods.Where(m => m.Name.StartsWith("set")))
            {
                sb.AppendLine($"{currentMethod.Name} will set field of {currentMethod.GetParameters().First().ParameterType}");
            }
